...
const ListOwner = () => {
    ...
    Get(`admin/owner?page=${currentPage}${queryParams}`);
    ...
    useEffect(() => {
        ...
        const query = (currentPage === 1 && params === '') ? '/owners' : `?page=${currentPage}${params}`;
        ...
        <AuthLayout>
            <div>
                ...
                <div className="items-center space-x-2 md:flex">
                        <Link to={'create'}>
                            <Button text="Add New Owner"/>
                        </Link>
                        <DatePickerField onChangeInput={(date) => setDate(date)} value={date} placeholder="Filter by date created"/>
                        <InputField onChangeInput={(e) => setSearchInput(e.target.value)} value={searchInput} placeholder="Search for owner" type="search"/>
                    </div>
                    {!isErrorRequest && (
                        <div className="mt-[3em] space-y-[3em]">
                                        ...
                                        <tbody className="relative">
                                            {getData ? getData?.items?.map((item: UserType, index: number)=>(
                                                ...
                                                <td className="flex px-4 py-3 space-x-2 capitalize cursor-pointer text-secondary hover:underline">
                                                    <Link to={`/owners/${item.uuid}/show`} className="flex items-center space-x-2">
                            ...
                            />
                        {getData?.items.length === 0 && <EmptyState page="owner"/>}
                        </div>
                    )}
            </div>
        </AuthLayout>

...
const ListEnforcer = () => {
    ...
    Get(`admin/enforcer?page=${currentPage}${queryParams}`);
    ...
    useEffect(() => {
        ...
        const query = (currentPage === 1 && params === '') ? '/enforcers' : `?page=${currentPage}${params}`;
    ...
    return (
        <AuthLayout>
            <div>
                <PageTitle title="Enforcers" />
                <div className="items-center space-x-2 md:flex">
                        <Link to={'create'}>
                            <Button text="Add New Enforcer"/>
                        </Link>
                        <DatePickerField onChangeInput={(date) => setDate(date)} value={date} placeholder="Filter by date created"/>
                        <InputField onChangeInput={(e) => setSearchInput(e.target.value)} value={searchInput} placeholder="Search for enforcer" type="search"/>
                    </div>
                    {!isErrorRequest && (
                        <div className="mt-[3em] space-y-[3em]">
                                        ...
                                        <tbody className="relative">
                                            {getData ? getData?.items?.map((item: UserType, index: number)=>(
                                                ...
                                                <td className="flex px-4 py-3 space-x-2 capitalize cursor-pointer text-secondary hover:underline">
                                                    <Link to={`/enforcers/${item.uuid}/show`} className="flex items-center space-x-2">
                            ...
                            />
                        {getData?.items.length === 0 && <EmptyState page="enforcer"/>}
                        </div>
                    )}
            </div>
        </AuthLayout>

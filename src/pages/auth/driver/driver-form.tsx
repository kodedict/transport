...
const DriverForm = ({
    ...
    const {
        ...
        useEffect(() => {driver && reset({
            first_name: driver.first_name,
            last_name: driver.last_name,
            phone_number: driver.phone_number,
        }); driver && setPermissions(driver.permissions)}, [driver])

    const Submit = async (data:any) => {
        data.permissions = permissions;
        data['uuid'] = driver?.uuid;
        data.email = data.email == '' ? undefined : data.email;
        //data.first_name = isEdit ? driver.first_name : data.first_name;
        //data.last_name = isEdit ? driver.last_name : data.last_name;
        const endpoint = isEdit ? `admin/driver/update` : 'admin/driver';
        const onSubmit = await Post({endpoint, payload: data, refreshEndpoint: `admin/driver`});
        if ( ! onSubmit )return;
        SuccessToast(`Driver ${isEdit ? 'updated' : 'created'} successfully`)
        if (!isEdit) {
            const emptyValues = EmptyFormInput(formSchema);
            reset(emptyValues)
            return
        }
        driver['email'] = onSubmit.email;
        setValue('email', '');
    }
        ...
        <form onSubmit={handleSubmit(Submit)} className="grid gap-5 mt-10">
            <div className="grid gap-5 md:grid-cols-2 p-5 themeRounded" style={{ boxShadow: 'rgba(149, 157, 165, 0.2) 0px 8px 24px' }}>
                ...
                <InputField 
                    ...
                    placeholder={isEdit ? driver?.email : 'Enter email'}
            ...
            <Button isLoading={requestLoading} type="submit" className="w-fit" text={`${isEdit ? 'Update' : 'Create'} Driver`}/>
        </form>
    )
}
